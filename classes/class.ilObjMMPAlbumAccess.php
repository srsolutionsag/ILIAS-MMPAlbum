...
class ilObjMMPAlbumAccess extends ilObjectPluginAccess
{
	...
	*/
	static function checkOnline($a_id)
	{
		...
		$set = $ilDB->query("SELECT is_online FROM rep_robj_xmma_data WHERE id = ".$ilDB->quote($a_id, "integer"));
	...
	*/
	static function getUpdateMode($a_id)
	{
		...
		$set = $ilDB->query("SELECT update_mode FROM rep_robj_xmma_data WHERE id = ".$ilDB->quote($a_id, "integer"));
		$rec = $ilDB->fetchAssoc($set);
